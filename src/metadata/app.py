...
def _calculate_metrics(df: pd.DataFrame) -> dict[str, float]:
    """Compute key performance metrics from the SOC-enriched DataFrame.

    Args:
        df: DataFrame with ``SOC``, ``Voltage_Filtered``, ``Current_Filtered``,
            and ``Temperature`` columns.

    Returns:
        A dict with rounded metric values.
    """
    avg_soc = float(df["SOC"].mean())
    avg_voltage = float(df["Voltage_Filtered"].mean())
    peak_temp = float(df["Temperature"].max())
    capacity_discharged = float((df["SOC"].iloc[0] - df["SOC"].iloc[-1]) * Q_CAPACITY_AH)

    energy_delivered = capacity_discharged * avg_voltage
    peak_discharge_current = float(df["Current_Filtered"].max())
    peak_regen_current = abs(float(df["Current_Filtered"].min()))
    voltage_sag = float(df["Voltage_Filtered"].iloc[0] - df["Voltage_Filtered"].min())
    min_voltage = float(df["Voltage_Filtered"].min())
    starting_temp = float(df["Temperature"].iloc[0])
    delta_temp = peak_temp - starting_temp
    avg_c_rate = float(df["Current_Filtered"].abs().mean() / Q_CAPACITY_AH)
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    Returns:
        A dict with ``status``, ``csv_output``, and ``json_output``.
    ...
    if not dest_bucket:
        raise ValueError("DEST_BUCKET environment variable is not set")
    ...
    csv_key = f"results/{drive_cycle}_{test_date}.csv"
    json_key = f"metadata/{drive_cycle}_{test_date}.json"
    ...
    return {
        "status": "SUCCESS",
        "csv_output": f"s3://{dest_bucket}/{csv_key}",
        "json_output": f"s3://{dest_bucket}/{json_key}",
    }
