...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    1. **Read** the validated CSV from S3 (key must start with ``validated/``).
    2. **Savitzky-Golay filter** — smooths ``Voltage`` and ``Current`` columns
       to reduce sensor noise while preserving signal shape.  Skipped when
       the DataFrame has fewer than 11 rows.
    3. **Min-max normalization** — scales ``Voltage_Filtered``,
       ``Current_Filtered``, and ``Temperature`` to [0, 1] using fixed
       physical bounds for Panasonic 18650PF cells.
    4. **Write** the processed CSV under the ``processed/`` prefix.
    ...
    Returns:
        ...
        - ``key`` (:class:`str`): The S3 key of the **processed** CSV
          (under the ``processed/`` prefix).
    ...
    # ------------------------------------------------------------------
    # 1. Read validated CSV from S3 into a pandas DataFrame
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 2. Apply Savitzky-Golay filter to Voltage and Current
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 4. Write the processed DataFrame back to S3 under the ``processed/``
    #    prefix.
    # ------------------------------------------------------------------
    if not validated_key.startswith("validated/"):
        raise ValueError(
            f"S3 key must start with 'validated/', got: {validated_key}"
        )
